...
class Maze:
    ...
    def draw_connect_cells(self, from_cell, compass_index):
        ...
        if compass_index == 0:
            pygame.draw.line(self.m_layer, colors.NO_COLOR, (x_pos, y_pos + 1),
                             (x_pos, y_pos + CELL_SIZE - 1))
        elif compass_index == 1:
            pygame.draw.line(self.m_layer, colors.NO_COLOR, (x_pos + 1,
                                                             y_pos + CELL_SIZE), (x_pos + CELL_SIZE - 1,
                                                                                  y_pos + CELL_SIZE))
        elif compass_index == 2:
            pygame.draw.line(self.m_layer, colors.NO_COLOR, (x_pos + CELL_SIZE,
                                                             y_pos + 1), (x_pos + CELL_SIZE,
                                                                          y_pos + CELL_SIZE - 1))
        elif compass_index == 3:
            pygame.draw.line(self.m_layer, colors.NO_COLOR, (x_pos + 1, y_pos),
                             (x_pos + CELL_SIZE - 1, y_pos))

    def draw_visited_cell(self, cell):
        x_pos, y_pos = self.x_y_pos(cell)
        if self.x_y_pos(cell)[0] != 0 or self.x_y_pos(cell)[1] != 0:
            pygame.draw.rect(self.s_layer, colors.PURPLE, pygame.Rect(x_pos, y_pos,
                                                                      CELL_SIZE, CELL_SIZE))

    def draw_backtracked_cell(self, cell):
        x_pos, y_pos = self.x_y_pos(cell)
        if self.x_y_pos(cell)[0] != 0 or self.x_y_pos(cell)[1] != 0:
            pygame.draw.rect(self.s_layer, colors.RED, pygame.Rect(x_pos, y_pos,
                                                                   CELL_SIZE, CELL_SIZE))
    ...
    def setup_maze_window(self):
        ...
        pygame.display.set_caption(
            'Maze Generating and Solving with DFS Algorithm')
        pygame.mouse.set_visible(0)
        self.background = self.background.convert()
        self.background.fill(colors.WHITE)
        self.m_layer = self.m_layer.convert_alpha()
        self.m_layer.fill(colors.NO_COLOR)
        self.s_layer = self.s_layer.convert_alpha()
        self.s_layer.fill(colors.NO_COLOR)

        # Draw grid lines
        for y in range(self.h_cells + 1):
            pygame.draw.line(self.m_layer, colors.BLACK, (0, y * CELL_SIZE),
                             (SCREEN_SIZE[0], y * CELL_SIZE))
        for x in range(self.w_cells + 1):
            pygame.draw.line(self.m_layer, colors.BLACK, (x * CELL_SIZE, 0),
                             (x * CELL_SIZE, SCREEN_SIZE[1]))

        # start at top-left and exit at bottom-right
        pygame.draw.rect(self.s_layer, colors.GREEN,
                         pygame.Rect(0, 0, CELL_SIZE, CELL_SIZE))
        pygame.draw.rect(self.s_layer, colors.BLUE, pygame.Rect(
